...
namespace ManagementCenter {
    class Server {
        ...
        private ConfigLoader ConfigLoaderRepair = new ConfigLoader();
        ...
        public Server(string config, string config2) {
            ...
            try {
                ...
                if(config2.Length > 0)
                    ConfigLoaderRepair.LoadConfig(config2);
        ...
        private void StartListening() {
            /*
            new Thread(() => {
                lock(nodeConnectionWaiter) {
                    Monitor.Wait(nodeConnectionWaiter);
                }

                foreach(HostConnection hostConnection in hostConnections) {
                    hostConnection.SendAllHosts();
                }
            }).Start();
            */
            while (true) {
        ...
        public void ResendMPLS() {
            //GUIWindow.PrintLog("Yo");
            foreach (RouterConnection routerConnection in routerConnections)
                routerConnection.SendMPLSTableAgain(ConfigLoaderRepair);
        }
        ...
        public void HandleClient(TcpClient client) {
            ...
            try {
                ...
                if(numOfHosts >= 7) {
                    lock (nodeConnectionWaiter) {
                        Monitor.PulseAll(nodeConnectionWaiter);
                    }
                }
                */
