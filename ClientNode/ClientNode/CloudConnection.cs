...
namespace ClientNode
{
    class CloudConnection
    {
        ...
        private void ReceiveMessages() {
            ...
            while (true) {
                try {
                    ...
                    GUIWindow.PrintLog("MPLS label " + frame.MPLSlabels.Pop().ToString() + " has been popped");
        ...
        public static void SendMessage(String message, String destination, String initialLabel) {

            if (connected) {
                Frame frame = new Frame(ClientIP, ClientPort, destination, 50000, message);
                GUIWindow.PrintLog("Pushing label '" + initialLabel + "' onto MPLS labels stack");
                frame.MPLSlabels.Push(initialLabel);
                byte[] bytes = SerializeObject(frame);

                stream.Write(bytes, 0, bytes.Length);
                GUIWindow.PrintLog("Message " + "\"" + message + "\"" + " sent to " + destination);
        ...
        public void DisconnectFromCloud() {
            Frame disconnectionFrame = new Frame("", 0, "", 0, "_disconnect_");
